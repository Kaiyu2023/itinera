...
function label(ctx: CanvasRenderingContext2D, P: Proj, lng: number, lat: number, text: string, kind: LabelKind = 'area') {
  const [x, y] = P(lng, lat);
  ctx.save();
...
function streetGrid(ctx: CanvasRenderingContext2D, P: Proj, lng0: number, lng1: number, lngStep: number, lat0: number, lat1: number, latStep: number) {
  ctx.strokeStyle = GRID;
  ctx.lineWidth = 1.2;
  for (let lng = lng0; lng <= lng1; lng += lngStep) {
    strokeLine(ctx, P, [[lng, lat1], [lng, lat0]], GRID, 1.2);
  }
  for (let lat = lat0; lat <= lat1; lat += latStep) {
    strokeLine(ctx, P, [[lng0, lat], [lng1, lat]], GRID, 1.2);
  }
}
...
function drawKyoto(ctx: CanvasRenderingContext2D, P: Proj, w: number, h: number) {
  ...
  fillPoly(ctx, P, [[135.790, 35.11], [135.802, 35.03], [135.797, 34.995], [135.806, 34.955], [135.86, 34.94], [135.86, 35.11]], HILL);
  fillPoly(ctx, P, [[135.63, 35.11], [135.676, 35.05], [135.668, 35.020], [135.660, 34.995], [135.63, 34.98]], HILL);
  fillPoly(ctx, P, [[135.777, 34.972], [135.797, 34.967], [135.80, 34.947], [135.775, 34.95]], HILL);
  // Kyoto Gyoen (Imperial Palace park)
  fillPoly(ctx, P, [[135.759, 35.029], [135.769, 35.029], [135.769, 35.017], [135.759, 35.017]], HILL);
  // street grid (central Kyoto really is a grid)
  streetGrid(ctx, P, 135.712, 135.792, 0.006, 34.972, 35.036, 0.005);
  // major roads
  ctx.lineCap = 'round';
  strokeLine(ctx, P, [[135.7513, 35.06], [135.7513, 34.955]], ROAD, 4);
  strokeLine(ctx, P, [[135.759, 35.055], [135.759, 34.95]], ROAD, 4);
  strokeLine(ctx, P, [[135.66, 35.0037], [135.80, 35.0037]], ROAD, 4);
  strokeLine(ctx, P, [[135.68, 34.9949], [135.80, 34.9949]], ROAD, 4);
  // rivers — Kamo, Katsura
  strokeLine(ctx, P, [[135.7737, 35.11], [135.7716, 35.02], [135.7719, 34.995], [135.768, 34.968], [135.764, 34.93]], RIVER, 7);
  strokeLine(ctx, P, [[135.655, 35.035], [135.678, 35.011], [135.700, 34.988], [135.720, 34.952], [135.729, 34.93]], RIVER, 8);
  // rail through Kyoto Station
  strokeLine(ctx, P, [[135.63, 35.004], [135.74, 34.990], [135.7588, 34.9858], [135.83, 34.992]], RAIL, 2, [7, 5]);
  label(ctx, P, 135.789, 35.020, 'Higashiyama');
  label(ctx, P, 135.678, 35.038, 'Arashiyama');
  label(ctx, P, 135.7790, 35.0025, 'Gion');
...
function drawTokyo(ctx: CanvasRenderingContext2D, P: Proj, w: number, h: number) {
  ...
  ctx.beginPath();
  traceSmooth(ctx, P, [
    [139.62, 35.30], [139.67, 35.38], [139.71, 35.44], [139.745, 35.50], [139.755, 35.53],
    [139.765, 35.555], [139.748, 35.575], [139.752, 35.60], [139.765, 35.615], [139.78, 35.622],
    [139.792, 35.632], [139.795, 35.648], [139.782, 35.662], [139.802, 35.673], [139.83, 35.663],
    [139.87, 35.672], [139.905, 35.66], [139.94, 35.62], [139.95, 35.52], [139.90, 35.40], [139.82, 35.28],
  ]);
  ctx.closePath();
  ctx.fill();
  ...
  fillPoly(ctx, P, [[139.688, 35.678], [139.703, 35.679], [139.706, 35.669], [139.697, 35.662], [139.686, 35.666]], HILL);
  fillPoly(ctx, P, [[139.744, 35.692], [139.760, 35.693], [139.760, 35.680], [139.746, 35.678]], HILL);
  fillPoly(ctx, P, [[139.706, 35.689], [139.716, 35.688], [139.714, 35.681], [139.704, 35.683]], HILL);
  fillPoly(ctx, P, [[139.766, 35.720], [139.776, 35.722], [139.778, 35.712], [139.768, 35.710]], HILL);
  // Sumida river down past Asakusa to the bay
  strokeLine(ctx, P, [[139.80, 35.73], [139.797, 35.710], [139.800, 35.698], [139.786, 35.680], [139.786, 35.665], [139.772, 35.648]], RIVER, 6);
...
function drawHakone(ctx: CanvasRenderingContext2D, P: Proj, w: number, h: number) {
  ...
  ctx.fillRect(0, 0, w, h);
  ridgeBlobs(ctx, P, [
    [139.02, 35.27, 70], [139.06, 35.26, 55], [138.99, 35.22, 80], [139.045, 35.20, 60],
    [139.10, 35.185, 55], [139.13, 35.26, 50], [138.98, 35.28, 60], [139.075, 35.29, 45],
  ]);
  // Sagami Bay, SE corner
  ctx.fillStyle = WATER;
  ctx.beginPath();
  traceSmooth(ctx, P, [[139.12, 35.13], [139.16, 35.16], [139.21, 35.19], [139.26, 35.24], [139.32, 35.27], [139.34, 35.13]]);
  ctx.closePath();
  ctx.fill();
  ctx.strokeStyle = COAST;
  ctx.lineWidth = 1.5;
  ...
  ctx.beginPath();
  traceSmooth(ctx, P, [
    [139.007, 35.242], [139.003, 35.228], [139.008, 35.213], [139.017, 35.198], [139.029, 35.190],
    [139.034, 35.203], [139.028, 35.221], [139.018, 35.234], [139.010, 35.244],
  ]);
  ctx.closePath();
  ctx.fill();
  ctx.strokeStyle = COAST;
  ...
  strokeLine(ctx, P, [[139.155, 35.256], [139.135, 35.245], [139.107, 35.232], [139.093, 35.226], [139.082, 35.233], [139.068, 35.240], [139.052, 35.246]], RAIL, 2, [7, 5]);
  strokeLine(ctx, P, [[139.048, 35.247], [139.028, 35.243], [139.012, 35.238]], RAIL, 1.5, [3, 4]);
  label(ctx, P, 139.018, 35.211, 'Lake Ashi', 'water');
  label(ctx, P, 139.24, 35.20, 'Sagami Bay', 'water');
...
function drawOsaka(ctx: CanvasRenderingContext2D, P: Proj, w: number, h: number) {
  ...
  ctx.beginPath();
  traceSmooth(ctx, P, [
    [135.45, 34.79], [135.425, 34.72], [135.437, 34.70], [135.442, 34.685], [135.428, 34.66],
    [135.405, 34.645], [135.378, 34.62], [135.348, 34.60], [135.328, 34.57], [135.30, 34.53],
    [135.272, 34.48], [135.252, 34.44], [135.246, 34.40], [135.26, 34.355], [135.10, 34.30], [135.05, 34.80],
  ]);
  ctx.closePath();
  ctx.fill();
  ctx.strokeStyle = COAST;
  ...
  fillPoly(ctx, P, [[135.222, 34.437], [135.246, 34.447], [135.266, 34.433], [135.242, 34.423]], PAPER);
  strokeLine(ctx, P, [[135.255, 34.438], [135.31, 34.457]], RAIL, 2, [5, 4]);
  // street grid
  streetGrid(ctx, P, 135.455, 135.545, 0.006, 34.625, 34.725, 0.005);
  // Yodo river + Dōtonbori canal
  strokeLine(ctx, P, [[135.565, 34.745], [135.52, 34.722], [135.49, 34.703], [135.457, 34.692], [135.44, 34.686]], RIVER, 7);
  strokeLine(ctx, P, [[135.478, 34.6693], [135.522, 34.6688]], RIVER, 3);
  // Osaka Castle park
  fillPoly(ctx, P, [[135.519, 34.693], [135.533, 34.693], [135.533, 34.681], [135.519, 34.681]], HILL);
  label(ctx, P, 135.498, 34.706, 'Umeda');
  label(ctx, P, 135.501, 34.660, 'Namba');
...
function drawJapan(ctx: CanvasRenderingContext2D, P: Proj, w: number, h: number) {
  ...
  const coast: Pt[] = [
    [134.90, 34.75], [135.10, 34.70], [135.28, 34.72], [135.42, 34.68], [135.46, 34.55], [135.40, 34.42],
    [135.26, 34.32], [135.10, 34.24], [135.06, 33.90], [135.30, 33.55], [135.75, 33.44], [136.10, 33.70],
    [136.30, 34.10], [136.50, 34.42], [136.72, 34.50], [136.60, 34.62], [136.55, 34.85], [136.65, 35.02],
    [136.85, 35.07], [136.95, 34.90], [136.98, 34.68], [137.15, 34.75], [137.32, 34.64], [137.60, 34.63],
    [138.00, 34.60], [138.25, 34.65], [138.38, 34.95], [138.50, 35.02], [138.55, 34.92], [138.60, 34.66],
    [138.75, 34.60], [138.90, 34.60], [138.98, 34.72], [139.08, 34.92], [139.15, 35.10], [139.25, 35.20],
    [139.40, 35.32], [139.55, 35.33], [139.62, 35.20], [139.68, 35.14], [139.72, 35.22], [139.78, 35.32],
    [139.82, 35.46], [139.90, 35.60], [140.00, 35.66], [140.08, 35.56], [140.05, 35.40], [139.98, 35.32],
    [139.95, 35.20], [140.02, 35.08], [140.10, 34.95],
  ];
  ctx.fillStyle = PAPER;
  ctx.beginPath();
  ...
  ctx.stroke();
  // Lake Biwa
  fillPoly(ctx, P, [[136.07, 35.02], [136.00, 35.18], [136.06, 35.38], [136.18, 35.44], [136.28, 35.30], [136.21, 35.10]], WATER);
  // mountain ridges
  ridgeBlobs(ctx, P, [
    [135.8, 35.65, 90], [136.6, 35.75, 110], [137.5, 35.6, 130], [138.25, 35.75, 90],
    [139.0, 35.75, 70], [135.95, 34.5, 55], [137.2, 35.1, 80],
  ]);
  // Mt Fuji
  {
  ...
  strokeLine(ctx, P, [
    [139.77, 35.68], [139.62, 35.47], [139.16, 35.26], [138.62, 34.98], [137.98, 34.77], [137.38, 34.77],
    [136.88, 35.10], [136.30, 35.05], [135.94, 34.99], [135.7588, 34.9858], [135.50, 34.70], [135.36, 34.66],
  ], RAIL, 2, [8, 6]);
  // Nagoya — basemap furniture, not a trip city
  {
    const [nx, ny] = P(136.90, 35.15);
    ctx.fillStyle = CITY_LABEL;
    ctx.beginPath();
    ctx.arc(nx, ny, 3, 0, 7);
    ctx.fill();
  }
  label(ctx, P, 136.90, 35.175, 'Nagoya', 'city');
  label(ctx, P, 138.60, 35.46, 'Mt Fuji', 'small');
...
function drawGeneric(center: LngLat, lngSpan: number) {
  return (ctx: CanvasRenderingContext2D, P: Proj, w: number, h: number) => {
    ...
    streetGrid(ctx, P, center.lng - lngSpan, center.lng + lngSpan, step, center.lat - lngSpan, center.lat + lngSpan, step * 0.8);
...
export function drawBasemap(ctx: CanvasRenderingContext2D, P: Proj, w: number, h: number, center: LngLat, lngSpan: number) {
  if (lngSpan > 1.2) {
    drawJapan(ctx, P, w, h);
    return;
  }
