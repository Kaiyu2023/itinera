...
export function LedgerTab() {
  ...
  const [surface, setSurface] = useState<
    { kind: 'add'; seed?: AddExpenseSeed } | { kind: 'settle'; confirm?: boolean } | null
  >(null);
    ...
    return (
      <div key={e.id} className={`card exp${flashed ? ' new-flash' : ''}`}>
        ...
        <div className="exp-amt">
          ...
          <div className="base">
            {e.currency === base ? 'entered in ' + base : '≈ ' + money(e.amount * e.fxRateToBase, base)}
          </div>
  ...
  return (
    <div className="m4-tab">
      <div className="m4-tab-head">
        ...
        <button type="button" className="btn accent" onClick={() => setSurface({ kind: 'add' })}>
          ＋ Add expense
        </button>
      </div>
      ...
      <div className="card" style={{ display: 'grid', gap: 6 }}>
        <div className="stat-row">
          <div className="stat">
            ...
            <div className="v">
              {money(totalDisplay, displayCurrency)} <small>≈ {money(totalBase, base)}</small>
            </div>
          </div>
          <div className="stat">
            ...
            <div className="v">
              ...
              <small>≈ {money(totalBase / memberCount, base)}</small>
            </div>
          </div>
          <div className="stat">
            ...
            return (
              <div key={b.userId} className="bal-row">
                ...
                <span className="bal-track">
                  <span className={`bal-fill ${pos ? 'pos' : 'neg'}`} style={{ width: `${width}%` }} />
                </span>
                <span className={`bal-amt ${pos ? 'pos' : 'neg'}`}>
                  {pos ? '+' : '−'}
                  {moneyWhole(Math.abs(b.net), base)}
                </span>
              </div>
            );
          })}
      ...
      <div className="card" style={{ marginBottom: 2 }}>
        ...
        <div className="filter-bar">
          ...
          <button type="button" className={`fchip${who === 'insplit' ? ' on' : ''}`} onClick={() => setWho('insplit')}>
            In my splits
          </button>
