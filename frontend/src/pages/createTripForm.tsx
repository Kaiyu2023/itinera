...
export function CreateTripForm({ onClose }: { onClose: () => void }) {
  ...
  const create = useMutation({
    mutationFn: () =>
      api.createTrip({ name: name.trim(), startDate, endDate, baseCurrency }),
    onSuccess: (trip) => {
    ...
    <SheetModal onClose={onClose}>
      <div className="exp-modal" role="dialog" aria-modal="true" aria-label="New trip">
        ...
        <div className="exp-body">
          <div className="frow">
            <label className="fl" htmlFor="trip-name">Name</label>
            <span className="fv">
              <input id="trip-name" className="tinp" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Spring in Kyushu" />
            </span>
          </div>

          <div className="frow">
            <label className="fl" htmlFor="trip-start">Dates</label>
            <span className="fv">
              <input id="trip-start" type="date" className="tinp date" value={startDate} onChange={(e) => setStartDate(e.target.value)} aria-label="Start date" />
          ...
          {startDate && endDate && endDate < startDate && (
            <div className="frow">
              <span className="fl" />
              <span className="fv"><span className="hint bad">⚠ The end date can't be before the start date.</span></span>
            </div>
          )}

          <div className="frow">
            <label className="fl" htmlFor="trip-cur">Currency</label>
            <span className="fv">
              <select id="trip-cur" className="tinp" value={baseCurrency} onChange={(e) => setBaseCurrency(e.target.value)}>
                {CURRENCIES.map((c) => (
        ...
        <div className="exp-foot">
          ...
          <button type="button" className="btn accent" disabled={!canSave || create.isPending} onClick={() => create.mutate()}>Create trip</button>
        </div>
      </div>
    </SheetModal>
  );
